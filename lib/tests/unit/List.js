...
describe('List', function() {
	...
	it('should render the image of given items', function() {
		list = new List({
			items: [{
				img: {src: '/myImageSrc', class: 'myImageClass'}
			}]
		}).render();
	...
	it('should render the content of given items', function() {
		list = new List({
			items: [{content: 'Item 1'}, {content: 'Item 2'}]
		}).render();

		var contents = list.element.querySelectorAll('li .list-text');
		assert.strictEqual('Item 1', contents[0].textContent);
		assert.strictEqual('Item 2', contents[1].textContent);
	});

	it('should render the help content of given items', function() {
		list = new List({
			items: [{help: 'Help 1'}, {help: 'Help 2'}]
		}).render();

		var helpNodes = list.element.querySelectorAll('li .list-help');
	...
	it('should render new items when the attribute is updated', function(done) {
		...
		list.items = [{content: 'New Item 1'}, {content: 'New Item 2'}];
