...
describe('Modal', function() {
  ...
  it('should render with header only', function() {
    ...
    var body = modal.getSurfaceElement('body').innerHTML;
    var footer = modal.getSurfaceElement('footer').innerHTML;
    assert.strictEqual('<a class="modal-close icon-12-close-long" data-onclick="hide"></a>header', header);
    assert.strictEqual('', body);
    assert.strictEqual('', footer);
  });
  ...
  it('should add overlay-visible class to body when overlay is visible', function(done) {
    modal = new Modal().render();
    async.nextTick(function() {
      assert.ok(dom.hasClass(document.body, 'overlay-visible'));
      done();
    });
  });

  it('should remove overlay-visible class to body when overlay is visible', function(done) {
    modal = new Modal().render();
    modal.overlay = false;
    async.nextTick(function() {
      assert.ok(!dom.hasClass(document.body, 'overlay-visible'));
  ...
  it('should only show modal overlay when modal is visible', function(done) {
    modal = new Modal({ visible: false }).render();
    assert.ok(!modal.overlayElement.parentNode);
    modal.visible = true;
    async.nextTick(function() {
      assert.ok(modal.overlayElement.parentNode);
